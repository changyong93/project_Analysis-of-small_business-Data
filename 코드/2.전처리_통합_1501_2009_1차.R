...
smallbz_sales <- smallbz_sales %>% 
  mutate(매출_월화수목 = 월요일_매출_금액+화요일_매출_금액+수요일_매출_금액+목요일_매출_금액,
         ...
         매출_연령대_2050 = 연령대_20_매출_금액+연령대_30_매출_금액+연령대_40_매출_금액+연령대_50_매출_금액,
         매출_남성 = 남성_매출_금액,
         매출_여성 = 여성_매출_금액,
         매출건수_월화수목 = 월요일_매출_건수+화요일_매출_건수+수요일_매출_건수+목요일_매출_건수,
         매출건수_금토일 = 금요일_매출_건수+토요일_매출_건수+일요일_매출_건수,
         매출건수_0614 = 시간대_건수.11_매출_건수+시간대_건수.14_매출_건수,
         매출건수_1421 = 시간대_건수.17_매출_건수+시간대_건수.21_매출_건수,
         매출건수_2106 = 시간대_건수.24_매출_건수+시간대_건수.06_매출_건수,
         매출건수_연령대_2050 = 연령대_20_매출_건수+연령대_30_매출_건수+연령대_40_매출_건수+연령대_50_매출_건수,
         매출건수_남성 = 남성_매출_건수,
         매출건수_여성 = 여성_매출_건수
         )
vars <- c("상권_코드","서비스_업종_코드","기준_년_코드","기준_분기_코드","서비스_업종_코드_명","당월_매출_금액","점포수",
          "매출_월화수목","매출_금토일","매출_0614","매출_1421","매출_2106","매출_연령대_2050","매출_남성","매출_여성",
          '당월_매출_건수',"매출건수_월화수목","매출건수_금토일","매출건수_0614","매출건수_1421","매출건수_2106",
          "매출건수_연령대_2050","매출건수_남성","매출건수_여성")
smallbz_sales <- smallbz_sales[,vars]
colnames(smallbz_sales)[c(3,4,5,6,7,16)] <- c("년도","분기","소분류","매출총액","점포수_추정매출","총매출건수")
...
smallbz_data <- smallbz_data[,c(1:8,12)]
...
#15.1분기 ~20.2분기의 22개의 분기 중 3회이상 임대료보다 매출이 적은 경우 제외
...
colnames(smallbz_pop)
#조작할 컬럼만 선택
vars <- c(1:4,534,535,8:29) #년도,분기,시군구명,행정동명,총유동인구수,시간대별유동인구수,요일별유동인구수
...
smallbz_pop <- smallbz_pop %>% 
  mutate(유동인구수_월화수목 = 월요일_유동인구_수+화요일_유동인구_수+수요일_유동인구_수+목요일_유동인구_수,
                   ...
                   유동인구수_연령대_2050 = 연령대_20_유동인구_수+연령대_30_유동인구_수+연령대_40_유동인구_수+연령대_50_유동인구_수,
                   유동인구수_남성 = 남성_유동인구_수,
                   유동인구수_여성 = 여성_유동인구_수
                   ) %>% 
  select(ADSTRD_CD,상권_코드,년도,분기,행정구역,행정동명,총_유동인구수,유동인구수_월화수목,유동인구수_금토일,
           유동인구수_0614,유동인구수_1421,유동인구수_2106,유동인구수_연령대_2050,유동인구수_남성,유동인구수_여성) %>% 
  group_by(ADSTRD_CD,상권_코드,년도,분기,행정구역,행정동명) %>% 
  summarise(총유동인구수 = sum(총_유동인구수),
                   유동인구수_월화수목 = sum(유동인구수_월화수목),
                   유동인구수_금토일 = sum(유동인구수_금토일),
                   유동인구수_0614 = sum(유동인구수_0614),
                   유동인구수_1421 = sum(유동인구수_1421),
                   유동인구수_2106 = sum(유동인구수_2106),
                   유동인구수_연령대_2050 = sum(유동인구수_연령대_2050),
                   유동인구수_남성 = sum(유동인구수_남성),
                   유동인구수_여성 = sum(유동인구수_여성)) %>% as.data.frame()
...
smallbz_faci <- smallbz_faci %>% 
  group_by(년도,분기,행정구역,행정동명,상권_코드) %>% summarise(집객시설_수 = sum(집객시설_수))

smallbz_total_1501_2009 <- merge(x = smallbz_total_1501_2009, y= smallbz_faci, by = c("년도","분기","행정구역","행정동명","상권_코드"),all.x =T)
...
#최종 데이터셋 컬럼 정리
vars <- c("상권_코드","년도","분기","행정구역","행정동명","대분류","중분류","소분류","점포수",
          "매출총액","매출_월화수목","매출_금토일","매출_0614","매출_1421","매출_2106","매출_연령대_2050","매출_남성","매출_여성",
          "총매출건수","매출건수_월화수목","매출건수_금토일","매출건수_0614","매출건수_1421","매출건수_2106","매출건수_연령대_2050","매출건수_남성","매출건수_여성",
          "총유동인구수","유동인구수_월화수목","유동인구수_금토일","유동인구수_0614","유동인구수_1421","유동인구수_2106","유동인구수_연령대_2050","유동인구수_남성","유동인구수_여성",
          "생존률_1년차","생존률_3년차","생존률_5년차","집객시설_수")
          
smallbz_total_1501_2009 <- smallbz_total_1501_2009[,vars]

summary(smallbz_total_1501_2009)
colnames(smallbz_total_1501_2009)
#NA처리, NA=>0 / 유동인구수 NA를 0으로 변환
na_0 <- function(x){ x <- ifelse(is.na(x)==T,0,x);return(x)}
smallbz_total_1501_2009[,c(28:36,40)] <- map_df(.x = smallbz_total_1501_2009[,c(28:36,40)],.f = na_0)
...
#상권 별 중분류에 따른 점포수 개수가 5개 이상인 지역만 선택
...
# #이상치 데이터가 있는 송파구 가락1동 반찬가게데이터 모두 삭제
...
#   geom_point(size = 5)
# smallbz_total_1501_2009 <- smallbz_total_1501_2009 %>%
#   filter(행정동명 != "가락1동" | !소분류 %in% c("반찬가게")) #"슈퍼마켓","육류판매",
# 
# #이상치 데이터가 있는 강남구 청담동 문구 데이터 중 상권코드 1000939 삭제
# smallbz_total_1501_2009 %>%
...
# 
# 
# #이상치 데이터가 있는, 상권코드 강남구 논현2동의 1000926, 1001111 상권 슈퍼마켓 데이터 제거
# smallbz_total_1501_2009 %>% 
#   mutate(년분기 = paste0(년도,"_",분기)) %>%
#   filter(년분기 != "2020_3" & 행정구역 == '강남구' & 행정동명 == "논현2동" & 소분류=="슈퍼마켓" & 
#               상권_코드 %in% c(1000926, 1001111)) %>%
#   ggplot(aes(x = 년분기, y=log(매출총액/점포수_추정매출),group = 상권_코드, color = as.factor(상권_코드)))+geom_point(size = 1.5)
# smallbz_total_1501_2009 %>% 
#   filter(행정구역 != '강남구' | 행정동명 != "논현2동" | 소분류!="슈퍼마켓" | !상권_코드 %in% c(1000926, 1001111))
# 
# #이상치 데이터가 있는 양천구 신정4동의 여관데이터 삭제
# smallbz_total_1501_2009 %>% 
#   mutate(년분기 = paste0(년도,"_",분기)) %>%
#   filter(년분기 != "2020_3" & 행정구역 == '양천구' & 행정동명 == "신정4동" & 소분류=="여관") %>% 
#   ggplot(aes(x = 년분기, y = log(매출총액/점포수_추정매출),group = 상권_코드, color = as.factor(상권_코드)))+geom_point(size = 2)
# smallbz_total_1501_2009 <- smallbz_total_1501_2009 %>% 
#   filter(행정구역 != '양천구' | 행정동명 != "신정4동" | 소분류!="여관")
...
save(smallbz_total_1501_2009,file = "dataset.rda")
# save(smallbz_total_1501_2009,file = "dataset_1501_2009_no_dealing_outliers.rda")
