...
#분기별 임대료로 변환# 0.5(임시)는 결제 방법 비율(카드 / 나머지)
...
#15.1분기 ~20.2분기의 22개의 분기 중 20회이상 임대료보다 매출이 적은 경우 제외
...
# smallbz_total_1501_2009 %>% mutate(매출총액_점포수 = 매출총액/점포수_추정매출) %>% 
#   filter(행정구역 == '강동구' & 행정동명 == "성내3동" & 소분류=="신발") %>% 
#   select(상권_코드,년도,분기,매출총액,매출총액_점포수,점포수_추정매출,임대료,행정동명) %>% 
#   arrange(상권_코드,년도,분기)

smallbz_total_1501_2009 <- smallbz_total_1501_2009 %>% filter(n>=20 | is.na(n)==T) %>% select(-c(n,임대료))
...
smallbz_pop <- smallbz_pop %>% 
  mutate(유동인구수비율_월화수목 = round((월요일_유동인구_수+화요일_유동인구_수+수요일_유동인구_수+목요일_유동인구_수)/(월요일_유동인구_수+화요일_유동인구_수+수요일_유동인구_수+목요일_유동인구_수+금요일_유동인구_수+토요일_유동인구_수+일요일_유동인구_수),digits = 4L),
                   유동인구수비율_금토일 = round((금요일_유동인구_수+토요일_유동인구_수+일요일_유동인구_수수)/(월요일_유동인구_수+화요일_유동인구_수+수요일_유동인구_수+목요일_유동인구_수+금요일_유동인구_수+토요일_유동인구_수+일요일_유동인구_수),digits = 4L),
                   유동인구수비율_0614 = round((시간대_2_유동인구_수+시간대_3_유동인구_수)/(시간대_1_유동인구_수+시간대_2_유동인구_수+시간대_3_유동인구_수+시간대_4_유동인구_수+시간대_5_유동인구_수+시간대_6_유동인구_수),digits = 4L),
                   유동인구수비율_1421 = round((시간대_4_유동인구_수+시간대_5_유동인구_수)/(시간대_1_유동인구_수+시간대_2_유동인구_수+시간대_3_유동인구_수+시간대_4_유동인구_수+시간대_5_유동인구_수+시간대_6_유동인구_수),digits = 4L),
                   유동인구수비율_2106 = round((시간대_6_유동인구_수+시간대_1_유동인구_수)/(시간대_1_유동인구_수+시간대_2_유동인구_수+시간대_3_유동인구_수+시간대_4_유동인구_수+시간대_5_유동인구_수+시간대_6_유동인구_수),digits = 4L),) %>% 
...
colnames(smallbz_total_1501_2009)
#최종 데이터셋 컬럼 정리
vars <- c("상권_코드","년도","분기","행정구역","행정동명","대분류","중분류","소분류","확진자수","점포수","매출총액",
          "매출비율_월화수목","매비율출_금토일","매출비율_0614","매출비율_1421","매출비율_2106","총_유동인구수","유동인구수비율_월화수목",
          "유동인구수비율_금토일","유동인구수비율_0614","유동인구수비율_1421","유동인구수비율_2106","생존률_1년차","생존률_3년차",
          "생존률_5년차","숙박시설_수","지하철역_수")

smallbz_total_1501_2009 <- smallbz_total_1501_2009[,vars]

#범주형 및 연속형 데이터 정리
vars_left <- 9:length(vars)
vars <- 2:8
...
smallbz_total_1501_2009[,vars_left] <- map_df(.x = smallbz_total_1501_2009[,vars_left],.f = as.numeric)

#파일 저장
setwd("C:/Users/ChangYong/Desktop/나노디그리/1.정규강의 학습자료/1차 프로젝트/소상공인/데이터")
save(smallbz_total_1501_2009,file = "dataset_1501_2009_임대료기준.rda")
